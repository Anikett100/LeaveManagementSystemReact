...
function UpdateLeave() {
  ...
  const [error, setError] = useState({ cc: '' });
  ...
  useEffect(() => {
    const fetchLeaveData = async () => {
      try {
        ...
        const selectedCC = Array.isArray(JSON.parse(leaveData.cc))
        ? JSON.parse(leaveData.cc).map((email) => ({
            label: email,
            value: email,
          }))
        : [];
        ...
        const daterange =
          moment(leaveData.fromdate).format("YYYY-MM-DD") +
